...
const Side_bar = () => {
    ...
    return (
        <div>
            ...
            {user ? (
                <div>
                    <Menubar model={user.data.role.name === "admin" ? menuAdmin : user.data.role.name === "moderator" ? menuModerator : menuUser} end={<Button label="Déconnexion" icon="pi pi-power-off" onClick={logout} />} />
                    </div>
            ) : (
                <div></div>
            )}
